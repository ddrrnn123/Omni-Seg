...
def mapping(img, output_dir, case_name):
    ...
    image = np.load(glob.glob(os.path.join(now_folder, 'Big_map.npy'))[0], allow_pickle=True)[:, :, :3]
    ...
    for ti in range(len(task_list)):
        ...
        #pred = np.load(glob.glob(os.path.join(now_folder, 'Big_pred.npy'))[0], allow_pickle=True)[:, :, :3]
    ...
    np.save(os.path.join(output_folder, 'merge_pred.npy'), big_pred, allow_pickle=True)
    np.save(os.path.join(output_folder, 'other_pred.npy'), check_pred, allow_pickle=True)
...
if __name__ == "__main__":
    ...
    getContour(big_slice_folder, output_dir, '0b8f60ca-2cb1-4e2b-83b8-a5ff6db96346_S-1909-007135_HE_2of2_1')
    ...
    for now_case in cases:
        ...
        if contour_map:
            ...
            print('step 3 contour:', end - start, 'seconds')
